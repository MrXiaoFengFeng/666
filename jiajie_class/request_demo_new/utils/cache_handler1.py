...
class Cache:
    ...
    def extract_data(cls, var_name, actual_data_key_word, actual_data_expression, response):
        ...
        cache_data = {var_name: _property}
        setattr(cls, var_name, _property)
        print(cls.__dict__)

    # 提取参数后需要保存参数
    @classmethod
    def set_cache(cls, cache: dict, response):
        # 重点重点来了，下述这段代码具有通用性，这个时候我们发现不止断言用到了，而且在参数提取也用到了，这个时候我们要把下述代码进行封装
        for var_name, var_expression in cache.items():
            # 先解析 cookies.Session
...
if __name__ == '__main__':
    ...
    # 现在来实现上述方法的具体过程
    Cache.set_cache(cache_desc=temp_extract, response=new_resp)
